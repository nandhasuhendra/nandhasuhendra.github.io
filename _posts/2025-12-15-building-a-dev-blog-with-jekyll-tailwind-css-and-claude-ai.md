...
That was me. For years I bookmarked "how to start a dev blog" articles without actually starting one. The excuses were always there: too busy, don't know what to write, can't pick a platform. The classic developer trap of over-engineering the setup before writing a single word.
...
Jekyll kept coming up. It's a static site generator where you write posts in Markdown, Jekyll compiles everything into plain HTML, and you get a fast lightweight site with zero database, zero server to manage, and zero hosting cost when paired with GitHub Pages.
...
Once I settled on Jekyll, I made decisions about the rest of the stack. Here's what I landed on and why:

**Tailwind CSS** - I didn't want to write CSS from scratch. Tailwind's utility classes let me design directly in HTML. More importantly, it meant I could ask AI to generate components and they'd just work, no custom class names to explain.
...
Here's the honest part. I know Ruby on Rails well. I know Go. I do not enjoy writing HTML and CSS from scratch. It's not that I can't, it's that I'd rather spend that time on content and architecture decisions.

So I paired up with Claude AI and treated it like a senior frontend engineer sitting next to me.

The workflow was simple:

1. I described what I wanted, like "a hero section with my name, tagline, and social links"
...
Was every suggestion perfect? No. I had to push back on a few things like overly complex markup, unnecessary abstractions, and wrong brand colors. But that's the same as working with any developer. You review, give feedback, and the output improves.

The most useful thing AI helped with was the parts I find genuinely boring: SEO meta tags, JSON-LD structured data, Open Graph tags, and RSS feeds. Stuff that matters for discoverability but requires no creative thought. I just said "add proper SEO support" and got back a complete `_includes/seo.html` and `_includes/json-ld.html` that I'd have spent half a day copying from Stack Overflow otherwise.
...
**What to actually write about** - No AI can tell you what experiences matter. The post about debugging duplicate Sidekiq jobs came from a real incident at work. That story is mine. The lesson learned is mine. AI can help format it, but it can't live it.
...
posts/index.html     # Posts listing with search and filter
...
GitHub Pages picks up the push, runs the Jekyll build, and deploys to your site automatically. No dashboards to log into. No deploy buttons to click. Just Git.

The entire setup costs zero dollars per month. For a personal blog with reasonable traffic, you will never need anything more.
...
If you're sitting on the fence about starting your own blog, just pick a stack, accept that it won't be perfect on day one, and ship it. The blog you have is infinitely better than the blog you're still planning.

---

**Summary:** I built this blog using Jekyll for static site generation, Tailwind CSS for styling, and GitHub Pages for free zero-maintenance hosting. Claude AI handled the boilerplate like SEO markup, icon setup, and component layouts, while I focused on architecture decisions and content. The result is a fast, simple site that costs nothing to run and deploys with a single `git push`. The most important lesson: start before you're ready, keep the stack boring, and write the thing.
